...
```txt
Use the TripLens Git/GitHub plugin's PR reviewer agent to inspect PR #1 with the GitHub CLI. Summarize the PR status, changed files, risky areas, and checks that should be run. Do not edit files.
```
